...
function writeReadyPosts(campaign) {
  ...
  for (const post of campaign.posts) {
    ...
    const body = [
      ...
      `# LOGO (attach brand mark): ${post.logoLocal || campaign.brandAssets?.logo?.localPath || "marketing/assets/brand/logo-square-800.png"}`,
      `# LOGO URL: ${post.logoUrl || campaign.brandAssets?.logo?.publicUrl || "https://ambulancenter.com/logo.png"}`,
      `# IMAGE (attach plane+ambulance): ${post.imageLocal || campaign.defaultHeroImage?.localPath || "ambulance-jet-handoff.jpg"}`,
      `# IMAGE URL: ${post.imageUrl || campaign.defaultHeroImage?.publicUrl || ""}`,
      `# WHATSAPP LINK: ${post.whatsappLink || campaign.contacts?.whatsappLink || ""}`,
      ``,
      `=== ENGLISH ===`,
      post.en,
      ``,
